...
def main():
    ...
    expression = ''
    network_roads = arcpy.FeatureClassToFeatureClass_conversion(
        sgid_roads,
        str(network_fgdb) + r'/NetworkDataset', 'Roads', expression
    )
    ...
    arcpy.CalculateField_management(
        urban_roads_selected, field='USEEXIST', expression='"Y"', expression_type='VB', code_block=''
    )
    # No - also set SPEED_LMT to 25 for these records as well
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION', "USEEXIST is null or USEEXIST <> '1'"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='USEEXIST', expression='"N"', expression_type='VB', code_block=''
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='SPEED_LMT', expression='25', expression_type='VB', code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        ...
        "CARTOCODE = '1' OR ((DOT_RTNAME = '0201P' OR DOT_RTNAME = '0201N') and CARTOCODE = '4') OR CARTOCODE = '2'"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='SPEED_LMT', expression='70', expression_type='VB', code_block=''
    )

    # 65 mph - major state and US highways
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION',
        "(CARTOCODE = '2' OR CARTOCODE = '3' OR CARTOCODE = '4' OR CARTOCODE = '5') AND USEEXIST = 'N'"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='SPEED_LMT', expression='65', expression_type='VB', code_block=''
    )

    # 55 mph - UDOT freeway collectors/feeders
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION', "DOT_RTNAME like '%C%' and (USEEXIST is null or USEEXIST <> 'Y')"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='SPEED_LMT', expression='55', expression_type='VB', code_block=''
    )

    # 40 mph - UDOT ramps
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION', "DOT_RTNAME like '%R%' and (USEEXIST is null or USEEXIST <> 'Y')"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='SPEED_LMT', expression='40', expression_type='VB', code_block=''
    )

    # 50 mph - invalid speeds on major local roads
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION', "(USEEXIST is null or  USEEXIST <> 'Y') AND ( CARTOCODE = '8')"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='SPEED_LMT', expression='55', expression_type='VB', code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION', "(USEEXIST is null or  USEEXIST <> 'Y') AND ( CARTOCODE = '9')"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='SPEED_LMT', expression='45', expression_type='VB', code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION',
        "(CARTOCODE = '2' OR CARTOCODE = '3' OR CARTOCODE = '4' OR CARTOCODE = '5' OR CARTOCODE = '8' OR CARTOCODE = '9') AND URBTRAFFIC = 'Y' AND NOT (DOT_RTNAME = '0201P' OR DOT_RTNAME = '0201N' OR DOT_RTNAME = '0154P' or DOT_RTNAME = '0154N' OR DOT_RTNAME = '0085P' or DOT_RTNAME = '0085N' or (DOT_RTNAME Like '0%' and (CARTOCODE = '2' or CARTOCODE = '4')) or DOT_RTNAME LIKE '0092PC%' or DOT_RTNAME LIKE '0092NC%' or (DOT_RTNAME LIKE '0089%' and DOT_RTPART = '10'))"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='SPEED_LMT', expression='40', expression_type='VB', code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION',
        "(CARTOCODE = '1' OR ((DOT_RTNAME = '0201P' OR DOT_RTNAME = '0201N') and CARTOCODE = '4') OR CARTOCODE = '2') and URBTRAFFIC = 'Y'"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='SPEED_LMT', expression='65', expression_type='VB', code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION', "DOT_RTNAME like '0092%' and DOT_RTNAME like '%C%'"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='SPEED_LMT', expression='65', expression_type='VB', code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION', "DOT_RTNAME like '0085%'"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='SPEED_LMT', expression='55', expression_type='VB', code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION', "DOT_RTNAME like '0154%'"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='SPEED_LMT', expression='55', expression_type='VB', code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION',
        "CARTOCODE = '1' or CARTOCODE = '2' or CARTOCODE = '4' or DOT_RTNAME like '%R%' or DOT_RTNAME like '%C%' or URBTRAFFIC = 'N'"
    )
    arcpy.CalculateField_management(
        urban_roads_selected,
        field='IMPED_MIN',
        expression='([SHAPE_Length]/1609 * 60) / [SPEED_LMT]',
        expression_type='VB',
        code_block=''
    )
    ...
    arcpy.CalculateField_management(
        urban_roads_selected,
        field='IMPED_MIN',
        expression='([SHAPE_Length]/1609 * 60) / [SPEED_LMT] * 1.5',
        expression_type='VB',
        code_block=''
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='IMPED_PED', expression='[SHAPE_Length] / 84', expression_type='VB', code_block=''
    )
    # select limited access roadways (ie: freeways/ramps) and multiply by 1.2
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION',
        "CARTOCODE = '1' OR ((DOT_RTNAME = '0201P' OR DOT_RTNAME = '0201N') and CARTOCODE = '4') OR CARTOCODE = '2' or CARTOCODE = '7'"
    )
    arcpy.CalculateField_management(
        urban_roads_selected,
        field='IMPED_MIN',
        expression='([SHAPE_Length]/1609 * 60) / [SPEED_LMT] * 1.2',
        expression_type='VB',
        code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION',
        "(DOT_RTNAME = '0215N' and DOT_F_MILE > DOT_T_MILE) or (DOT_RTNAME = '0215P' and DOT_F_MILE < DOT_T_MILE)"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='ONEWAY', expression='1', expression_type='VB', code_block=''
    )
    # set oneway to '2'
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION',
    ...
    arcpy.CalculateField_management(
        urban_roads_selected, field='ONEWAY', expression='2', expression_type='VB', code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION',
        "(DOT_RTNAME = '0080P' or DOT_RTNAME = '0084P' or DOT_RTNAME = '0070P' or (DOT_RTNAME = '0201P' and DOT_RTPART = '2' ) or (DOT_RTNAME = '0007P' and DOT_RTPART = '2')) and START_X < END_X"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='ONEWAY', expression='1', expression_type='VB', code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION',
        "(DOT_RTNAME = '0080N' or DOT_RTNAME = '0084N' or DOT_RTNAME = '0070N' or DOT_RTNAME = '0201N' or DOT_RTNAME = '0007N') and START_X > END_X"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='ONEWAY', expression='1', expression_type='VB', code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION',
        "(DOT_RTNAME = '0015P' or (DOT_RTNAME = '0152P' and DOT_RTPART ='2') or (DOT_RTNAME = '0154P' and DOT_RTPART ='2') or DOT_RTNAME = '0067P' or (DOT_RTNAME = '0189P' and DOT_RTPART ='2') or (DOT_RTNAME = '0191P' and DOT_RTPART ='2') or (DOT_RTNAME = '0089P' and (DOT_RTPART ='4' or DOT_RTPART ='7' or DOT_RTPART ='9' or DOT_RTPART ='11'))) and START_Y < END_Y"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='ONEWAY', expression='1', expression_type='VB', code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION',
        "(DOT_RTNAME = '0015N' or DOT_RTNAME = '0152N' or DOT_RTNAME = '0154N' or DOT_RTNAME = '0067N' or DOT_RTNAME = '0189N' or DOT_RTNAME = '0191N ' or DOT_RTNAME = '0040N' or DOT_RTNAME = '0006N' or DOT_RTNAME = '0089N' or (DOT_RTNAME = '0040P' and DOT_RTPART ='2') or (DOT_RTNAME = '0006P' and DOT_RTPART ='3')) and START_Y > END_Y"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='ONEWAY', expression='1', expression_type='VB', code_block=''
    )
    ...
    arcpy.CalculateField_management(
        urban_roads_selected, field='T_F_IMP_MIN', expression='[IMPED_MIN]', expression_type='VB', code_block=''
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='F_T_IMP_MIN', expression='[IMPED_MIN]', expression_type='VB', code_block=''
    )
    ...
    arcpy.CalculateField_management(
        urban_roads_selected, field='T_F_IMP_MIN', expression='[IMPED_MIN] * 100', expression_type='VB', code_block=''
    )
    ...
    arcpy.CalculateField_management(
        urban_roads_selected, field='F_T_IMP_MIN', expression='[IMPED_MIN] * 100', expression_type='VB', code_block=''
    )
    ...
    urban_roads_selected = arcpy.SelectLayerByAttribute_management(
        'network_roads_lyr', 'NEW_SELECTION',
        "CARTOCODE = '1' or CARTOCODE = '2' or CARTOCODE = '4' or DOT_RTNAME like '%R%' or DOT_RTNAME like '%C%'"
    )
    arcpy.CalculateField_management(
        urban_roads_selected, field='NETSUBTYPE', expression='1', expression_type='VB', code_block=''
    )
    arcpy.CalculateField_management(
    ...
    arcpy.CalculateField_management(
        urban_roads_selected, field='EXCL_WALK', expression='N', expression_type='VB', code_block=''
    )
...
def import_RoadsIntoNetworkDataset(sgid_roads_to_import, network_roads):
    ...
    with arcpy.da.SearchCursor(sgid_roads_to_import, '*',
...
def generate_scratch_data(directory):
    ...
    unioned_fc = arcpy.Union_analysis(
        in_features=
        "'Database Connections/internal@SGID@internal.agrc.utah.gov.sde/SGID.DEMOGRAPHIC.UrbanAreasCensus2010' #;'Database Connections/internal@SGID@internal.agrc.utah.gov.sde/SGID.BOUNDARIES.Municipalities' #",
        out_feature_class=str(directory) + '/NetworkBuild_scratchData.gdb/UrbanAreasMuni_Union',
    ...
    return arcpy.Dissolve_management(
        in_features=unioned_fc,
        out_feature_class=str(directory) + '/NetworkBuild_scratchData.gdb/UrbanAreasMuni_Union_Dissolved',
        dissolve_field='',
        statistics_fields='',
